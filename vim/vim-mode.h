...
//Compare search_string with the character at a given position in the buffer
uint64_t compare_char(Arena *arena, EditorCtx *ctx, uint64_t pos, String8 search_string) {
  EditorOffsetRange current_range = {0};
  current_range.first_off = pos;
  current_range.last_off = pos + 1;
  String8 curr_char = {0};
  ed_string_at_range(arena, ctx, &current_range, &curr_char);
  return str8_compare(curr_char, search_string);
}

//Move num_cursors amount of cursors to given pos; number of positions should equal num_cursors
void move_to(Arena *arena, VimCmdBufferList *cmd_lst, EditorCmd *cmd, uint64_t *pos, uint64_t num_cursors) {
  
  cmd->cmd = ED_NavMoveCursorTo;
  cmd->byte_offsets.array = pos;
  cmd->byte_offsets.size = num_cursors;
  push_vim_cmd_entry(arena, cmd_lst, cmd);
}

//Add Update selection flag and then call move_to
void move_to_selection(Arena *arena, VimCmdBufferList *cmd_lst, EditorCmd *cmd, uint64_t *pos, uint64_t num_cursors) {
  cmd->flags = ED_FLG_UpdateSelection;
  move_to(arena, cmd_lst, cmd, pos, num_cursors);
}

//Given a cursor offset search for search_string on that line, search_forward == true check right else check left
uint64_t search_in_line(Arena *arena, EditorCtx *ctx, uint64_t cursor_offset, String8 search_string, int search_forward) {
    uint64_t current_line = ed_line_at_offset(ctx, cursor_offset);
    uint64_t move_to_byte = cursor_offset;
    
    EditorOffsetRange offset_range = {0};
    ed_byte_range_at_line(ctx, current_line, &offset_range);
    
    uint64_t direction = search_forward ? 1 : -1;
    uint64_t start = search_forward ? cursor_offset + 1 : cursor_offset - 1;
    uint64_t end =  search_forward ? offset_range.last_off : offset_range.first_off;
  
    for (;;) {
      if (compare_char(arena, ctx, start, search_string) == 0) {
        move_to_byte = start;
        break;
      }
      int should_continue = search_forward ? start <= end : start >= end;
      if (!should_continue) {
        break;
      }
...
void vim_process_vis(EditorCtx* ctx, VimProcessorState* vim_state, char c) {
  ...
  //fx and tx properties
  EditorCursorArray cursors = {0};
  String8 search_string = str8(&c, 1);
  ...
  case VIM_SUBCMD_f:
    {
      ...
      uint64_t* cursor_positions = push_array_no_zero(scratch.arena, uint64_t, cursors.size);
      
      for EachIndex(i, cursors.size) {
        cursor_positions[i] = search_in_line(scratch.arena, ctx, cursors.array[i].cursor_off, search_string, 1) + pos_offset;
      }
      move_to(scratch.arena, &cmd_lst, &cmd, cursor_positions, cursors.size);
    }
    break; 
  case VIM_SUBCMD_T:
    pos_offset = 1;
  case VIM_SUBCMD_F:
    {
      ...
      for EachIndex(i, cursors.size) {
        cursor_positions[i] = search_in_line(scratch.arena, ctx, cursors.array[i].cursor_off, search_string, 0) + pos_offset;
      }
      move_to(scratch.arena, &cmd_lst, &cmd, cursor_positions, cursors.size);
    }
    break;
  ...
  case VIM_SUBCMD_dt: 
    {
      ...
      move_to_selection(scratch.arena, &cmd_lst, &cmd, cursor_positions, cursors.size); 
  ...
  case VIM_SUBCMD_yt:
    {
      ...
      //Gather initial cursor positions to return to
      EditorCursorArray cursors_base = {0};
      ed_cursor_ranges(scratch.arena, ctx, &cursors_base);
      uint64_t* cursor_base_positions = push_array_no_zero(scratch.arena, uint64_t, cursors_base.size);
      for EachIndex(i, cursors_base.size) {
        cursor_base_positions[i] = cursors_base.array[i].cursor_off;
      }
      ...
      push_vim_cmd_entry(scratch.arena, &cmd_lst, &cmd);
      move_to(scratch.arena, &cmd_lst, &cmd, cursor_base_positions, cursors.size); 
    }
    break;
  default:
    // Command processing.
    switch (c) {
      ...
      case 'z':
        ...
        break;
      case 'f':
        vim_state->sub_cmd = VIM_SUBCMD_f;
        break;
      case 'F':
        vim_state->sub_cmd = VIM_SUBCMD_F;
        break;
      case 't':
        vim_state->sub_cmd = VIM_SUBCMD_t;
        break;
      case 'T':
        vim_state->sub_cmd = VIM_SUBCMD_T;
        break;
  ...
  switch (prev_sub_cmd) {
    ...
    case VIM_SUBCMD_f:
      {
        ...
        for EachIndex(i, cursors.size) {
          cursor_positions[i] = search_in_line(scratch.arena, ctx, cursors.array[i].cursor_off, search_string, true) + pos_offset;
        }
        move_to(scratch.arena, &cmd_lst, &cmd, cursor_positions, cursors.size);
      }
      break; 
    ...
    case VIM_SUBCMD_F:
      {
        ...
        for EachIndex(i, cursors.size) {
          cursor_positions[i] = search_in_line(scratch.arena, ctx, cursors.array[i].cursor_off, search_string, false) + pos_offset;
        }
        move_to(scratch.arena, &cmd_lst, &cmd, cursor_positions, cursors.size);
      }
      break;
    ...
    default:
      switch (c) {
        ...
        case 'f':
          if (prev_sub_cmd == VIM_SUBCMD_d) {
            vim_state->sub_cmd = VIM_SUBCMD_df;
          } else if (prev_sub_cmd == VIM_SUBCMD_y) {
            vim_state->sub_cmd = VIM_SUBCMD_yf;
          }else {
            vim_state->sub_cmd = VIM_SUBCMD_f;
          }
          break;
        ...
        case 't':
          if (prev_sub_cmd == VIM_SUBCMD_d) {
            vim_state->sub_cmd = VIM_SUBCMD_dt;
          } else if (prev_sub_cmd == VIM_SUBCMD_y) {
            vim_state->sub_cmd = VIM_SUBCMD_yt;
          }else {
            vim_state->sub_cmd = VIM_SUBCMD_t;
          }
          break;
